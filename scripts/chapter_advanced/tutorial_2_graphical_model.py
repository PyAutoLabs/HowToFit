...
In the `info` above, the whole distinction between a `centre` shared by every dataset and a `centre` per dataset is
carried by an index range, the shared `centre` being printed once under a `0 - 4` heading instead of once per
dataset.
"""
